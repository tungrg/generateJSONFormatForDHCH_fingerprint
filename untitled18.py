...
Original file is located at
    https://colab.research.google.com/drive/1XM0W4nvNXJw0E8jb-flLivqW3PU5F1EI
"""

!gdown https://drive.google.com/uc?id=1_i4ci3YCGyoN606ZuyvprgQzNqpC_ZBG > dhcp_fingerprints.conf

import json
import re

file1 = open('/content/dhcp_fingerprints.conf', 'r')
  ...
  while(index < len(Lines)):
    ...
    if ("vendor_id" in Lines[index]):
      ...
      while("EOT" not in Lines[index]):
        ...
        index+=1
      index+=1
    print("vendor list: ",vendorIDList)
    fingerprintsList = []
...
result = {}
for i in OSChildList:
  tempDictResult = {"description": i.name}
  if len(i.vendorList):
    tempDictResult.update({"vendor_id": i.vendorList})
  if len(i.fingerprint):
    tempDictResult.update({"fingerprints": i.fingerprint})
